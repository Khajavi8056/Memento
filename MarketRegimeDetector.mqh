...
private:
    ...
    int                m_fractal_n;         // دوره فرکتال
    double             m_consolidation_factor; // ضریب تشخیص رنج
    double             m_fractal_atr_filter_factor; // ضریب فیلتر ATR برای فرکتال‌ها
    int                m_atr_period_consolidation; // دوره ATR
    int                m_atr_handle;        // هندل ATR
    SwingPoint         m_swing_highs[];     // نقاط چرخش بالا
    SwingPoint         m_swing_lows[];      // نقاط چرخش پایین
    long               m_last_processed_bar_time; // زمان آخرین پردازش
    CLogManager*       m_logger;            // اشاره‌گر لاگ (هشدار: ریسک Dangling Pointer در صورت مدیریت نادرست چرخه عمر)

    // کامنت فارسی: یافتن نقاط چرخش با فرکتال و فیلتر ATR برای سرعت بیشتر
    void FindSwingPoints(const MqlRates &rates[], const double &atr_buf[], const int bars_to_check)
    {
        long current_bar_time = (long)SeriesInfoInteger(m_symbol, m_period, SERIES_LASTBAR_DATE);
        if(current_bar_time == m_last_processed_bar_time) return;
        ...
        for(int i = m_fractal_n; i < bars_to_check - m_fractal_n; i++)
        {
            ...
            // فیلتر ATR برای کاهش نویز (قابل تنظیم توسط کاربر)
            if(is_swing_high && m_fractal_atr_filter_factor > 0 && i > 0 && i < ArraySize(atr_buf))
            {
                if(MathAbs(rates[i].high - rates[i-1].high) <= atr_buf[i] * m_fractal_atr_filter_factor)
                    is_swing_high = false;
            }
            if(is_swing_low && m_fractal_atr_filter_factor > 0 && i > 0 && i < ArraySize(atr_buf))
            {
        ...
        m_logger.Log(StringFormat("نقاط چرخش یافت شد: Highs=%d, Lows=%d", ArraySize(m_swing_highs), ArraySize(m_swing_lows)));
    }
...
public:
    ...
    // کامنت فارسی: تحلیل ساختار بازار با اولویت سرعت و دقت
    ENUM_STRUCTURE_STATE Analyze(const MqlRates &rates[], const double &atr_buf[], const int bars_to_process)
    {
        ...
        {
            m_logger.Log("خطا: نقاط چرخش کافی برای تحلیل ساختار یافت نشد");
            return STRUCTURE_UNDEFINED;
        }

        SwingPoint last_h = m_swing_highs[highs_count-1];
        SwingPoint prev_h = m_swing_highs[highs_count-2];
        SwingPoint last_l = m_swing_lows[lows_count-1];
        SwingPoint prev_l = m_swing_lows[lows_count-2];

        double last_swing_range = MathAbs(last_h.price - last_l.price);
        double atr = atr_buf[0];
        if(last_swing_range < m_consolidation_factor * atr)
        {
            m_logger.Log("ساختار رنج تشخیص داده شد (تراکم)");
        ...
        m_logger.Log("ساختار بازار نامشخص است");
        return STRUCTURE_CONSOLIDATION_RANGE;
    }

    int GetSwingHighs(SwingPoint &result_array[])
    {
        if(ArraySize(m_swing_highs) > 0) ArrayCopy(result_array, m_swing_highs);
        return ArraySize(m_swing_highs);
    }

    int GetSwingLows(SwingPoint &result_array[])
    {
...
private:
    ...
    bool DetectDivergence(const MqlRates &rates[])
    {
        ...
        int bars_available = ArraySize(rates);
        int h1_idx = bars_available - 1 - h1.bar_index;
        int h2_idx = bars_available - 1 - h2.bar_index;
        int l1_idx = bars_available - 1 - l1.bar_index;
        int l2_idx = bars_available - 1 - l2.bar_index;

        int max_idx = (int)MathMax(MathMax(h1_idx, h2_idx), MathMax(l1_idx, l2_idx)) + 1;
        if(max_idx >= ArraySize(m_rsi_buf)) return false;

        if(h1.price > h2.price && m_rsi_buf[h1_idx] < m_rsi_buf[h2_idx]) return true; // واگرایی نزولی
        if(l1.price < l2.price && m_rsi_buf[l1_idx] > m_rsi_buf[l2_idx]) return true; // واگرایی صعودی
        return false;
    }
