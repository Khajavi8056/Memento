...
//| ۴. بعد از باز شدن هر معامله در تابع OpenTrade، این خط را اضافه کنید: |
//| TrailingStop.AddTradeState(m_trade.ResultDeal(), sl_price);
//| ۵. در انتهای تابع OnTimer (یا OnTick) اکسپرت خود، این خط را اضافه کنید: |
//| TrailingStop.Process();
//| ۶. در تابع OnTradeTransaction اکسپرت خود، برای معاملات بسته شده، این خط را اضافه کنید: |
//| TrailingStop.Cleanup(deal_ticket);
//| |
//================================================================================//
//================================================================//
...
public:
 ...
 // ✅ تابع جدید برای ذخیره حالت معامله
 void AddTradeState(ulong ticket, double initial_sl);
};
// --- مخرب کلاس ---
...
void CTrailingStopManager::Process()
{
 ...
 // یک کپی از آرایه حالت‌ها می‌گیریم تا در صورت تغییر، حلقه به مشکل نخوره
 STradeState current_trade_states[];
 ArrayCopy(current_trade_states, m_trade_states);
for(int i = ArraySize(current_trade_states) - 1; i >= 0; i--)
 {
  ulong ticket = current_trade_states[i].ticket;
  if(!PositionSelectByTicket(ticket)) continue;
  if(PositionGetInteger(POSITION_MAGIC) != m_magic_number) continue;
  
  // --- پیدا کردن ایندکس اصلی معامله در آرایه مِین ---
...
void CTrailingStopManager::AddTradeState(ulong ticket, double initial_sl)
{
...
// ✅✅✅ تابع جدید: پاکسازی حالت معامله بسته شده ✅✅✅
void CTrailingStopManager::Cleanup(ulong ticket)
{
 int idx = FindTradeStateIndex(ticket);
 if(idx != -1)
 {
  ArrayRemove(m_trade_states, idx, 1);
...
double CTrailingStopManager::CalculateIchimokuSL(string symbol, ENUM_POSITION_TYPE type)
{
 ...
 // ✅ منطق اصلاح شده: برای BUY دنبال خط زیر قیمت می‌گردیم، برای SELL دنبال خط بالای قیمت
